...
namespace Kyoob.Entities
{
    ...
    public abstract class Entity
    {
        ...
        private const int NumberOfSurroundingChunks = 9; // 3 * 3
        ...
        {
            if ( IsOnGround && Position.Y < Terrain.MaxEntityHeight )
            {
        ...
        private void FindSurroundingChunks()
        {
            ...
            for ( int x = -1; x <= 1; ++x )
            {
                for ( int z = -1; z <= 1; ++z )
                {
                    center.X = Position.X + x * ChunkData.SizeXZ * 0.5f;
                    center.Z = Position.Z + z * ChunkData.SizeXZ * 0.5f;
