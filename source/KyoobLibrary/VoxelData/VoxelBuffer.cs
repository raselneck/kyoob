...
namespace Kyoob.VoxelData
{
    ...
    public sealed class VoxelBuffer : IDisposable
    {
        ...
        public void SetData( BlockVertex[] data )
        {
            lock ( _mutex )
            {
                DisposeNoLock();
                if ( data.Length > 0 )
                {
                    ...
                    {
                        _vertexBuffer = new VertexBuffer( Game.Instance.GraphicsDevice, BlockVertex.VertexDeclaration, data.Length, BufferUsage.None );
                    }
